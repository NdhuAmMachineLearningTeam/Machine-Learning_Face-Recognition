...
# 心得：
### 410411325 林皓翔
<p>到了這一步反而有點不知道該說些甚麼了，不過整個完成後真的蠻有成就感的，一來是人臉辨識這個主題本來就相當的吸引人，且秀出圖片也遠比只有數據的分析來的有效果，二來是真的花上了不少的時間，突破不少困難後才終於到了現在這一步，過程中既充實又有趣。也在進行這份專案的過程中多接觸、多學到了許多新的套件，不過這邊有點可惜的是原本混淆矩陣在Jupyter Notebook上運行時是會把有值的元素用黃色底Mark起來的，所以一下就可以找到辨識錯誤的地方和其結果，但移到Markdown上後就不會顯示了。提到這個，這學期其實還有一項很大的收穫，在老師上課介紹到IPython Notebook時我才知到原來他的每一格區塊不一定只能輸入程式碼，還可以使用Mark down進行文字的說明，明明自己一直都是用這個在寫程式的，沒想到卻都沒發現，這下總算理解他名稱上叫Notebook的原因了。知道這點後真的方便了許多，這次的Mark down原型就是用Jupyter Notebook產生出來的，如果要直接從零開始完成到這份報告的程度恐怕得多花上好幾倍的時間吧，所以能學會並實際在這邊練習過真是太好了。</p>

### 410411245 陳本預
<p>報告主要用PCA把資料降維度，然後選用SVM做辨識器</p>
<p>在這次的工作中，我學會了correlation coefficient, covariance, covarince matrix的定義</p>
<p>學會了PCA的實作做法和PCA的數學原理</p>
<p>然後也看了SVM的原理跟實際運作方法</p>
<p>學會了linear kernel的數學原理</p>
< p >大慨就是這些</ p >
### 410419陳宇凱
<p>這次期末報告主題出來時我覺得很複雜，要用到老師上課所說的降維，與辨識方法，我在一開始實作時一直遇到問題常常在python遇到很多錯誤，後來在皓翔的幫助<p>下，他教會了我使用sklearn的方法以及應用，還問了很多次哈哈。在這次的功課中我也因此學到了很多sklearn的應用與python的語法(雖然現在還在摸熟)，PCA的<p>原理大致上都懂了會瞭解他如何應用(感謝功課2)，至於SVM我還需要加強因為還是有許多不太了解的地方，謝謝老師的這堂課讓我學到了很多東西，雖然常常遇到了<p>語法錯誤就卡了很久，我想我會先繼續加強對python的理解，之後再來繼續學習深度學習等更加進階的方向。謝謝我們的組員以及老師。
